...
export default function MenuScreen({ onNavigate, isBluetoothConnected }) {
  return (
    <div className="flex flex-col h-full text-white">
      {/* 헤더에 뒤로가기 버튼과 타이틀, 사용 가이드(ℹ️) 정보 버튼 배치 */}
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          <button 
            className="p-2 bg-white/5 hover:bg-white/10 rounded-full border border-white/10 cursor-pointer transition-colors active:scale-95"
            onClick={() => onNavigate('cover')}
            title="통합 홈으로"
          >
            <ChevronLeft size={18} className="text-slate-300" strokeWidth={2.5} />
          </button>
          <h1 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-white to-purple-200 drop-shadow-[0_0_15px_rgba(255,255,255,0.4)]">
            TONO-i
          </h1>
        </div>
        <button 
